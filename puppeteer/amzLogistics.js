...
const getStarleaseData = async (props) => {
  ...
  await page
    .goto('https://www.amazonlogistics.com/yms/shipclerk/#/carrierDashboard', {
      waitUntil: 'networkidle2',
    })
    .catch(async (err) => {
      result = {
        success: false,
        message: err?.message || 'Error occurred with "going to assetslist"!',
      };
      await browser.close();
      throw result;
    });
  ...
  await page
    .waitForSelector('button.yms-button-primary-alt.ng-isolate-scope')
    .catch(async (err) => {
      result = {
        success: false,
        message:
          err?.message || 'Error occurred with waiting "download button"!',
      };
  ...
  await page
    .click('button.yms-button-primary-alt.ng-isolate-scope')
    .catch(async (err) => {
      result = {
        success: false,
        message:
  ...
  // Form Date
  var filename = __dirname + '/../download/websiteData/carrier-dashboard.csv';
  const formData = new FormData();
  formData.append('file', fs.createReadStream(filename));
  formData.append('trip-stage', 'amazon-logistics');

  console.log('[file] => sending request ...');
