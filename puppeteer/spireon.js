...
const getStarleaseData = async (props) => {
  ...
  await page.hover('#exportSelector-1567-btnInnerEl');
  await page.waitForTimeout(20000).catch(async (err) => {
    result = {
      success: false,
      message: err?.message || 'Error occurred with waiting!',
  ...
  await page.waitForSelector('#menuitem-1569-textEl').catch(async (err) => {
    result = {
      success: false,
  ...
  await page.click('#menuitem-1569-textEl').catch(async (err) => {
    result = {
      success: false,
      message:
  ...
  // Form Date
  var filename = __dirname + '/../download/websiteData/Map_Assets.csv';
